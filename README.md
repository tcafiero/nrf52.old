#Arduino  Core for IoThingsWare nRF52 Platform
...
### Optional (Core Development): Adafruit nRF52 BSP via git
 ...
 4. Create a folder named ```hardware```, if it does not exist, and change directories to it
 5. Expand  IoThingsWare.zip
 6. Restart the Arduino IDE
 7. Once the BSP is installed, select 'IoThingsWare nRF52 Feather' from the Tools -> Board menu, which will update your system config to use the right compiler and settings for the nRF52.

### Third Party Tools

#### nrfutil

The IoThingsWare nRF52 BSP includes a [python wrapper](https://github.com/NordicSemiconductor/pc-nrfutil)
for Nordic's `nrfutil`, which is used to flash boards. Go into the BSP folder
(`hardware/IoThingsWare/tools/nrfutil-0.5.2`), and run the following to make
...
- Select `Tools > Board > IoThingsWare nrf52`
- Select `Tools > Programmer > J-Link for IoThingsWare nrf52`
...
which in turn is based on the [Arduino SAMD Core](https://github.com/arduino/ArduinoCore-samd) and also based on the BSP [Adafruit NRF52](https://www.adafruit.com/package_adafruit_index.json) by [Adafruit](https://www.adafruit.com/).
