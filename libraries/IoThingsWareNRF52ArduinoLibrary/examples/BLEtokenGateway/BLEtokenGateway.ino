...
char format[] = "{\"gateway\":\"%llx\",\"token\":{\"address\":\"%s\",\"name\":\"%s\",\"rssi\":%d}}";
...
class MyAdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks {
    void onResult(BLEAdvertisedDevice advertisedDevice) {
      ...
      if (advertisedDevice.haveName() && (!strcmp(advertisedDevice.getName().c_str(), "JustFoundIt") || !strcmp(advertisedDevice.getName().c_str(), "IoThingsWare"))) {
        sprintf(output, format, chipid, (char*)advertisedDevice.getAddress().toString().c_str(), (char*)advertisedDevice.getName().c_str(), advertisedDevice.getRSSI());
        q.push(output);
        Serial.print("<");
      }
    }
